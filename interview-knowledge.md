...
<table>
  ...
  <tbody>
    <tr>
      <td style="text-align:center">&#x539F;&#x578B;&#x94FE;&#x7EE7;&#x627F;</td>
      <td style="text-align:left">&#x5B50;&#x7C7B;&#x7684;&#x539F;&#x578B;&#x662F;&#x7236;&#x7C7B;&#x7684;&#x5B9E;&#x4F8B;</td>
      <td
    ...
    <tr>
      <td style="text-align:center">&#x6784;&#x9020;&#x7EE7;&#x627F;</td>
      <td style="text-align:left">&#x5B50;&#x7C7B;&#x8C03;&#x7528;&#x7236;&#x7C7B;&#x6784;&#x9020;&#x51FD;&#x6570;</td>
      <td
      style="text-align:left">+</td>
        <td style="text-align:left">-</td>
        <td style="text-align:left">+</td>
    ...
    <tr>
      <td style="text-align:center">&#x5B9E;&#x4F8B;&#x7EE7;&#x627F;</td>
      <td style="text-align:left">&#x7236;&#x7C7B;&#x5B9E;&#x4F8B;&#x4F5C;&#x4E3A;&#x5B50;&#x7C7B;&#x5B9E;&#x4F8B;&#x8FD4;&#x56DE;</td>
      <td
      style="text-align:left">+</td>
        <td style="text-align:left">+</td>
        <td style="text-align:left">-</td>
    ...
    <tr>
      <td style="text-align:center">&#x62F7;&#x8D1D;&#x7EE7;&#x627F;</td>
      <td style="text-align:left">&#x5B50;&#x7C7B;&#x539F;&#x578B;&#x62F7;&#x8D1D;&#x7236;&#x7C7B;&#x5B9E;&#x4F8B;&#x7684;&#x5C5E;&#x6027;&#x548C;&#x65B9;&#x6CD5;</td>
      <td
      style="text-align:left">+</td>
        <td style="text-align:left">+</td>
        <td style="text-align:left">+</td>
    ...
    <tr>
      <td style="text-align:center">&#x7EC4;&#x5408;&#x7EE7;&#x627F;</td>
      <td style="text-align:left">&#x8C03;&#x7528;&#x7236;&#x7C7B;&#x6784;&#x9020;&#xFF0C;&#x5B50;&#x7C7B;&#x539F;&#x578B;&#x662F;&#x7236;&#x7C7B;&#x5B9E;&#x4F8B;</td>
      <td
      style="text-align:left">+</td>
        <td style="text-align:left">+</td>
        <td style="text-align:left">&#xFF1F;</td>
    ...
    <tr>
      <td style="text-align:center">&#x5BC4;&#x751F;&#x7EC4;&#x5408;&#x7EE7;&#x627F;</td>
      <td style="text-align:left">/</td>
      <td style="text-align:left">+</td>
      <td style="text-align:left">+</td>
      <td style="text-align:left">&#xFF1F;</td>
...
{% hint style="info" %}
ES6的继承super实际执行的是 `父类.prototype.constructor.call(this)`
{% endhint %}
...
* 深拷贝——完全一致的对象，切不影响原始对象
  ...
  * 自定义拷贝（包装对象 data、number、string 和 boolean、RegExp 需要使用 valueof\(\) 转成原始值）
...
8）let/const/var 区别以及 ES5 中怎么实现块级作用域
...
{% hint style="info" %}
async中如果非异步函数，则认为顺序执行，只有await中为异步，后面的代码才进入微任务队列。
{% endhint %}
...
![](.gitbook/assets/h5-jie-gou-yu-yi-hua-.png)
...
![](.gitbook/assets/h5-xin-zeng-.png)

### 9. 图片格式

![](.gitbook/assets/tu-pian-ge-shi-.png)
...
![](.gitbook/assets/qing-qiu-tou-.png)
...
3. 验证证书的合法性
4. 验证通过客户端再生成一个随机值，并用公钥对随机值进行加密。
5. 客户端将加密后的密文发送给服务器。
6. 服务器通过私钥对密文进行解密得到对称加密的秘钥。
7. 此时两端都拥有了三个随机值，按照之前约定的加密方式可进行密文通信。
...
session： 1. 用户输入其登录信息 2. 服务器验证信息是否正确，并创建一个session，然后将其存储在数据库中 3. 服务器为用户生成一个sessionId，将具有sesssionId的Cookie将放置在用户浏览器中 4. 在后续请求中，会根据数据库验证sessionID，如果有效，则接受请求 5. 一旦用户注销应用程序，会话将在客户端和服务器端都被销毁

token： 1. 用户输入其登录信息 2. 服务器验证信息是否正确，并返回已签名的token 3. token储在客户端，例如存在local storage或cookie中 4. 之后的HTTP请求都将token添加到请求头里 5. 服务器解码JWT，并且如果令牌有效，则接受请求 6. 一旦用户注销，令牌将在客户端被销毁，不需要与服务器进行交互一个关键是令牌是无状态的，服务器不需要保存令牌或当前session的记录。

4）cookie 的过期时间？

通过 Expires / Max-age 设置过期时间，两者区别在于 Expires 为绝对时间，Max-age 为相对时间，并且 Max-Age 优先级更高。
...
![](.gitbook/assets/cdnnocahche.png)
...
![](.gitbook/assets/jin-cheng-xian-cheng-.png)
...
![](.gitbook/assets/qian-duan-neng-li-.jpg)
